...
export const messageEmbeddingsRelations = relations(messageEmbeddings, ({ one }) => ({
  message: one(messages, { fields: [messageEmbeddings.messageId], references: [messages.id] }),
  business: one(business, { fields: [messageEmbeddings.businessId], references: [business.id] }),
  conversation: one(conversations, { fields: [messageEmbeddings.conversationId], references: [conversations.id] }),
}));
